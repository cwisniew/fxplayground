...
public class HTMLCodeEditor extends StackPane {
    ...
    private static final URL BASE_HREF = HTMLCodeEditor.class.getResource("/net/rptools/encounter/ui/editor/");


    static {
        try (Scanner scanner = new Scanner(
                HTMLCodeEditor.class.getResourceAsStream("/net/rptools/encounter/ui/editor/HTMLCodeEditor.html"),
                StandardCharsets.UTF_8.name()
        )) {
            editingHTMLTemplate = scanner.useDelimiter("\\A").next();
        }
        try (Scanner scanner = new Scanner(
                HTMLCodeEditor.class.getResourceAsStream("/net/rptools/encounter/ui/editor/EmptyHTMLFile.html"),
                StandardCharsets.UTF_8.name()
        )) {
    ...
    public HTMLCodeEditor(String code) {
